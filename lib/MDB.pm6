...
class MdbHandle is repr('CStruct') {
    ...
    has CArray[uint8]               $!pg-buf;          # unsigned char pg_buf[MDB_PGSIZE];
    has CArray[uint8]               $!alt-pg-buyf;     # unsigned char alt_pg_buf[MDB_PGSIZE];
    has uint32                      $!num-catalog;     # unsigned int  num_catalog;
    has Pointer                     $!catalog;         # GPtrArray       *catalog;
...
class HashTable is repr('CStruct') {
  has uint8 $!a;
}

class MdbProperties is repr('CStruct') {
	has Str                $!name; # gchar *name;
	has Pointer[HashTable] $!hash; # GHashTable	*hash; ### FIX
}

class MdbCatalogEntry is repr('CStruct') {
	has Pointer[MdbHandle]     $!mdb;         # MdbHandle	*mdb;
	has CArray[uint8]          $!object-name; # char object-name[MDB-MAX-OBJ-NAME+1]; ### FIX
	...
	has Pointer[MdbProperties] $!props;       # GArray		*props; # GArray of MdbProperties */
	has Pointer[uint32]        $!columns; # GArray		*columns; ### FIX
...
class MdbAny is repr('CUnion') {
	...
	has CArray[uint8] $!s; # char	s[256];
}

class PtrArray is repr('CStruct') { ### FIX
  has uint8 $a;
}

#struct S-MdbTableDef; # forward definition */
class dbColumn is repr('CStruct') {
	has CArray[uint8]         $!name;            # char		name[MDB-MAX-OBJ-NAME+1]; ### FIX
	...
	has Pointer[HashTable]    $!properties;      # GHashTable	*properties;
	has uint32                $!num-sargs;       # unsigned int	num-sargs;
	has Pointer[PtrArray]     $!sargs;           # GPtrArray	*sargs;
	has Pointer[PtrArray]     $!idx-sarg-cache;  # GPtrArray	*idx-sarg-cache;
...
typedef struct {
	...
	int offset;
	int len;
	guint16 idx-starts[2000];	
	unsigned char cache-value[256];
} MdbIndexPage;

typedef int (*MdbSargTreeFunc)(MdbSargNode *, gpointer data);

#define MDB-MAX-INDEX-DEPTH 10

typedef struct {
	int cur-depth;
	guint32 last-leaf-found;
	int clean-up-mode;
	MdbIndexPage pages[MDB-MAX-INDEX-DEPTH];
} MdbIndexChain;

typedef struct S-MdbTableDef {
	MdbCatalogEntry *entry;
	char	name[MDB-MAX-OBJ-NAME+1];
	unsigned int    num-cols;
	GPtrArray	*columns;
	unsigned int    num-rows;
	int	index-start;
	unsigned int    num-real-idxs;
	unsigned int    num-idxs;
	GPtrArray	*indices;
	guint32	first-data-pg;
	guint32	cur-pg-num;
	guint32	cur-phys-pg;
	unsigned int    cur-row;
	int  noskip-del;  # don't skip deleted rows */
	# object allocation map */
	guint32  map-base-pg;
	size-t map-sz;
	unsigned char *usage-map;
	# pages with free space left */
	guint32  freemap-base-pg;
	size-t freemap-sz;
	unsigned char *free-usage-map;
	# query planner */
	MdbSargNode *sarg-tree;
	MdbStrategy strategy;
	MdbIndex *scan-idx;
	MdbHandle *mdbidx;
	MdbIndexChain *chain;
	MdbProperties	*props;
	unsigned int num-var-cols;  # to know if row has variable columns */
	# temp table */
	unsigned int  is-temp-table;
	GPtrArray     *temp-table-pages;
} MdbTableDef;
...
struct mdbindex {
	...
	guint32		first-pg;
	int		num-rows;  # number rows in index */
	unsigned int	num-keys;
	short	key-col-num[MDB-MAX-IDX-COLS];
	unsigned char	key-col-order[MDB-MAX-IDX-COLS];
	unsigned char	flags;
	MdbTableDef	*table;
};
...
typedef struct {
	...
	int siz;
	int start;
	unsigned char is-null;
	unsigned char is-fixed;
	int colnum;
	int offset;
} MdbField;
...
sub mdb-open(str, MdbFileFlags) returns MdbHandle is native('mdb') is symbol('mdb_open') { * }
